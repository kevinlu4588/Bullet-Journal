...
import cs3500.pa05.controller.handlers.file.OpenTemplateHandler;
...
/**
 * Represents a start from template task controller.
 */
public class StartFromTemplateController extends AbstractSubController {
  ...
  /**
   * Instantiates a new start from template controller.
   *
   * @param stage   the stage
   * @param journal the journal
   * @param master  the master controller
   */
  public StartFromTemplateController(Stage stage, JournalInteractable journal,
  ...
  private void loadRealPopUp() {
    ...
    try {
      FXMLLoader loader = new FXMLLoader(
          this.getClass().getClassLoader().getResource("StartFromTemplateDialog.fxml"));
      loader.setController(this);
      scene = loader.load();
    } catch (IOException e) {
      System.out.println("loader failed");
    }
    EventHandler<ActionEvent> finish = new OpenTemplateHandler(this.stage, this.popup,
